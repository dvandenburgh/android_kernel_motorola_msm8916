...
replace $old $ver -- $KERNEL_DIR/arch/arm/configs/surnia_defconfig
...
rm -f ~/android/kernel/upload/surnia/*
cd flash_zip
zip -r ../arch/arm/boot/fire_kernel.zip ./
today=$(date +"-%d%m%Y")
mv ~/android/kernel/motorola/msm8916/arch/arm/boot/fire_kernel.zip ~/android/kernel/upload/surnia/FireKernel-$anv-$dev-v$ver$today.zip
}
